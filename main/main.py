...
HELP_TEXTS = {
    ...
    "dashboard": "<h2>Dashboard Help</h2><p>The Dashboard provides a comprehensive overview of your study progress using several interactive charts:</p><ul><li><b>Pie Chart:</b> Shows how many hours each subject from your schedule takes. Each segment displays the percentage of each subject according to your schedule.</li><li><b>Heatmap:</b> All topics are listed on the left. If a subject from your ECampus box is related to a given topic, the heatmap will highlight it. The heatmap is also divided into weeks.</li><li><b>Study Timer:</b> Track the amount of time you study each week by entering the hours next to each day and clicking 'Save'. The data will be shown as a bar chart. PLEASE DON'T USE TIMER FOR MORE THAN 1 HOUR (The connection will be lost). You can update the data at any time, as it is saved with your account. Additionally, you can set a timer in real time. After stopping the timer, the data will be added to the bar chart, which becomes stacked. Data is also stored automatically. You can reset your diagram if needed.</li><li><b>Scatter Plot Chart:</b> The diagram is divided into four sections; the purpose of each section is described directly on the diagram labels. Some statistical analysis, such as Z-Score, is provided. The size of each point reflects its value. To access the scatter plot, go to the Dashboard section and select the 'Diagram' button.</li></ul><p>Interact with the visual elements (hover, click, filter), and review the detailed statistics provided.</p>",
